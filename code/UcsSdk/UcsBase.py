...
from UcsMeta import MoClassId, MethodClassId
#from UcsSdk import MoMeta, MethodMeta
import MoMeta, MethodMeta
...
class UcsBase:
    ...
    def ToXml(self, options):
        ...
        w.append(self.WriteXml(w, options))
        return w.toxml()
...
class ManagedObject(UcsBase):
    ...
    def setattr(self, key, value):
        ...
        if (UcsUtils.FindClassIdInMoMetaIgnoreCase(self.classId) != None):
            ...
            if (key in UcsUtils.GetUcsPropertyMetaAttributeList(self.classId)):
    ...
    def __setattr__(self, key, value):
        ...
        if ((key == "classId") or (UcsUtils.FindClassIdInMoMetaIgnoreCase(self.classId) == None) or (key not in UcsUtils.GetUcsPropertyMetaAttributeList(self.classId))):
            self.__dict__[key] = value
            return
    ...
    def WriteXml(self, w=None, option=None, elementName=None):
        ...
        if (option == WriteXmlOption.Dirty and not self.IsDirty()):
            return
        if w is None:
            x=Element(self.propMoMeta.xmlAttribute)
        else:
            if elementName == None:
                x = SubElement(w,self.propMoMeta.xmlAttribute)
            else:
            ...
            for at in UcsUtils.GetUcsPropertyMetaAttributeList(self.classId):
                ...
                if (atMeta.access == UcsPropertyMeta.Internal):
                    ...
                    if (getattr(self, at) != None):
                        x.set(atMeta.xmlAttribute,getattr(self, at))
        #Adding XtraProperties from object into Xml query document
        for xtraProp in self.__dict__['XtraProperty']:
            x.set(UcsUtils.WordL(xtraProp),self.__dict__['XtraProperty'][xtraProp])
        self.childWriteXml(x, option)
#        x_child = self.childWriteXml(x, option)
#        for xc in x_child:
#            if (xc != None):
#                x.append(xc)
        return x
    ...
    def LoadFromXml(self, element, handle):
        ...
        if element.attrib :
            for attr_name, attr_value in element.attrib.iteritems():
                attr = UcsUtils.WordU(attr_name)
                if (UcsUtils.FindClassIdInMoMetaIgnoreCase(self.classId) != None):
                    if (attr in UcsUtils.GetUcsPropertyMetaAttributeList(self.classId)):
                        self.setattr(attr, str(attr_value))
                    else:
                        self.setattr(attr, str(attr_value))
                else:
                    self.setattr(UcsUtils.WordU(attr), str(attr_value))

            if self.getattr("Rn") is None and self.getattr("Dn") != None:
        ...
        if child_elements:
            for child_element in child_elements:
                ...
                if child_element.tag in self.propMoMeta.fieldNames:
                    pass
                #TODO: Need code analysis.
                c = ManagedObject(UcsUtils.WordU(child_element.tag))
                self.child.append(c)
    ...
    def WriteXml(self, w=None, option=None, elementName=None):
        ...
        for at in UcsUtils.GetUcsPropertyMetaAttributeList(self.classId):
            ...
            if atMeta.isComplexType:
                if (getattr(self, at) != None):
                    self.__dict__[at].WriteXml(x, option, UcsUtils.WordL(at))
                    #x.append(self.__dict__[at].WriteXml(x, option, UcsUtils.WordL(at)))
            ...
            for attr_name, attr_value in element.attrib.iteritems():
                ...
                if (attr in UcsUtils.GetUcsPropertyMetaAttributeList(self.classId)):
                    ...
                    if ((atMeta.io == "Input") or (atMeta.isComplexType)):
                        continue
                    self.setattr(attr, str(attr_value))
                elif (attr_name in externalMethodAttrs):
                    self.setattr(attr_name, str(attr_value))
...
class UcsUtils:
    ...
    def IsValidClassId(classId):
        ...
        if classId in MoClassId or classId in MethodClassId:
            return True
        return False
    ...
    def GetUcsPropertyMeta(classId, key):
        ...
        #return None
        if classId in MoClassId:
            #mo_import = "MoMeta.%sMeta" %(classId)
            mo_import = MoMeta.__name__ + ".%sMeta" %(classId)
            from_list = "%sMeta" %(classId)
            moMeta =__import__(mo_import, globals(), locals(), [from_list])
            moPropMeta = moMeta.PROPERTY_DICT
            if key in moPropMeta:
                return moPropMeta[key]
        return None
    ...
    def GetUcsMethodMeta(classId, key):
        ...
        #return None
        if classId in MethodClassId:
            #method_import = "MethodMeta.%sMeta" %(classId)
            method_import = MethodMeta.__name__ + ".%sMeta" %(classId)
            from_list = "%sMeta" %(classId)
            methodMeta =__import__(method_import, globals(), locals(), [from_list])
            methodPropMeta = methodMeta.PROPERTY_DICT
            if key in methodPropMeta:
                return methodPropMeta[key]
        return None
    ...
    def GetUcsPropertyMetaAttributeList(classId):
        ...
        if (nci != None):
            ...
            #mo_import = "MoMeta.%sMeta" %(nci)
            mo_import = MoMeta.__name__ + ".%sMeta" %(nci)
            from_list = "%sMeta" %(nci)
            moMeta =__import__(mo_import, globals(), locals(), [from_list])
            moPropMeta = moMeta.PROPERTY_DICT
            attrList = moPropMeta.keys()
        ...
        if (nci != None):
            ...
            #method_import = "MethodMeta.%sMeta" %(nci)
            method_import = MethodMeta.__name__ + ".%sMeta" %(nci)
            from_list = "%sMeta" %(nci)
            methodMeta =__import__(method_import, globals(), locals(), [from_list])
            methodPropMeta = methodMeta.PROPERTY_DICT
            attrList = methodPropMeta.keys()
    ...
    def IsPropertyInMetaIgnoreCase(classId, key):
        ...
        if classId in MoClassId:
            ...
            moMeta =__import__(mo_import, globals(), locals(), [from_list])
            moPropMeta = moMeta.PROPERTY_DICT
            for prop in moPropMeta:
                if (prop.lower() == key.lower()):
                    return moPropMeta[prop]
        ...
        if classId in MethodClassId:
            ...
            methodMeta =__import__(method_import, globals(), locals(), [from_list])
            methodPropMeta = methodMeta.PROPERTY_DICT
            for prop in methodPropMeta:
                if (prop.lower() == key.lower()):
                    return methodPropMeta[prop]
        return None
    ...
    def GetJavaInstallationPath():
        ...
        # Get JavaPath for Windows
            ...
            if not path:
                ...
                if not os.path.exists(path):
                    raise UcsValidationException("javaws.exe is not installed on System at path <%s>." %(path))
    ...
    def GetJavaVersion():
        ...
        except: subprocess.check_output = UcsUtils.check_output
        
        java_ver_full_str = subprocess.check_output(["java", "-version"],stderr=subprocess.STDOUT)
...
class GenericMO(ManagedObject):
    ...
    def __init__(self, mo, option):
        ...
        #w = xml.dom.minidom.Document()
        #w.append(mo.WriteXml(w, option))
        w = mo.WriteXml(option=option)

        #doc = parseString(w.toxml())
        doc = ET.tostring(w)
        root_element = ET.fromstring(doc)
        self.LoadFromXml(root_element, mo.handle)
...
#            self.dn = node.getAttribute(NamingPropertyId.DN)
        ...
        self.WriteToAttributes(element)
        ...
        if child_elements:
            for child_element in child_elements:
                ...
                child.LoadFromXml(child_element)
...
#                c.LoadFromXml(childNode)
    ...
    def WriteXml(self, w=None, option=None, elementName=None):
        ...
        if w is None:
            x=Element(self.classId)
        else:
            if elementName == None:
                x = SubElement(w,self.classId)
            else:
                x = SubElement(w,elementName)

        for prop in self.__dict__['properties']:
            x.set(UcsUtils.WordL(prop),self.__dict__['properties'][prop])
        self.childWriteXml(x, option)
