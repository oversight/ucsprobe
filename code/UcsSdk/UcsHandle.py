...
class UcsHandle:
	...
	def Uri(self):
		...
		return ("%s://%s%s" % (("https","http")[self._noSsl == True], self._name, (":" + str(self._port),"")[(((self._noSsl == False) and  (self._port == 443)) or ((self._noSsl == True) and (self._port == 80)))]))
	...
	def XmlQuery(self, method, options, dumpXml=None):
		...
		if ((self._transactionInProgress == True) and (method.propMoMeta.xmlAttribute in [NamingId.CONFIG_CONF_MO, NamingId.CONFIG_CONF_MOS])):
			...
			try:				
				...
				w=method.WriteXml(option=options)
				
				uri = self.Uri() + '/nuova'
				#print self._noSsl, self._port
				#print uri
				if (dumpXml in _AffirmativeList):
					print '%s ====> %s' % (self._ucs, ET.tostring(w))
				
				if (self._proxy is None):
					if(self._noSsl):
						req = urllib2.Request(url=uri,data=ET.tostring(w))
				...
				response.LoadFromXml(root_element, self)
				return response
			except Exception, e:
				raise
		
	def XmlRawQuery(self, xml_str, dumpXml=None):
		...
		uri = self.Uri() + '/nuova'
		if (dumpXml in _AffirmativeList):
			print '%s ====> %s' % (self._ucs, xml_str)

		w = ET.fromstring(xml_str)
	...
	def Login(self, name, username=None, password=None, noSsl=None, port=None, dumpXml=None, proxy=None, autoRefresh=YesOrNo.FALSE):
		"""
		...
		from MoMeta.FirmwareRunning import FirmwareRunning
		import getpass
...
#			self._port = 443
	...
	def StartKvmSession(self, serviceProfile = None, blade = None, rackUnit = None, frameTitle = None, dumpXml=None):
		...
		from MoMeta.MgmtIf import MgmtIf
	...
	def StartGuiSession(self):
		...
		try:
			...
			if javawsPath != None:
				...
				java_str = UcsUtils.GetJavaVersion()
				if re.match(r'1.7',java_str):
					if int(java_str.rsplit('_')[1]) > 45:
						debug_str='\t<property name="jnlp.ucsm.log.show.encrypted" value="true"/>'
					else:
						debug_str='\t<property name="log.show.encrypted" value="true"/>'
				else:
					debug_str='\t<property name="log.show.encrypted" value="true"/>'
	...
	def BackupUcs(self, type, pathPattern, timeoutSec = 600, preservePooledValues = False, dumpXml=None):
		...
		from MoMeta.TopSystem import TopSystem
		from MoMeta.MgmtBackup import MgmtBackup
	...
	def ImportUcsBackup(self, path=None, merge=False, dumpXml=False):
		...
		from Ucs import ConfigConfig
		from MoMeta.MgmtImporter import MgmtImporter
	...
	def SendUcsFirmware(self,path=None,dumpXml=False):
		...
		from UcsBase import WriteUcsWarning, UcsUtils, ManagedObject, WriteObject, UcsUtils, UcsValidationException, UcsException
		from Ucs import ConfigConfig
		from MoMeta.FirmwareDownloader import FirmwareDownloader
	...
	def GetTechSupport(self, pathPattern , ucsManager=False, ucsMgmt=False, chassisId=None, cimcId=None, adapterId=None, iomId=None, fexId=None, rackServerId=None, rackAdapterId=None, timeoutSec=600, removeFromUcs=False, dumpXml=None):
		...
		"""
		from UcsBase import WriteUcsWarning, UcsUtils, ManagedObject, WriteObject, UcsUtils, UcsValidationException, UcsException
		from MoMeta.SysdebugTechSupport import SysdebugTechSupport
		from MoMeta.SysdebugTechSupportCmdOpt import SysdebugTechSupportCmdOpt
		from Ucs import ConfigConfig
		import os
	...
	def GetManagedObject(self, inMo=None, classId=None, params=None, inHierarchical=False, dumpXml=None):
		...
		from MoMeta.LsServer import LsServer
	...
	def ApeSetVmediaMounts(self, inChassisId, inCount, inSlotId, inVMediaSet, dumpXml=None):
		from UcsBase import ExternalMethod,UcsUtils,WriteUcsWarning
...
def CompareManagedObject(referenceObject, differenceObject, excludeDifferent=YesOrNo.FALSE, includeEqual=YesOrNo.FALSE, noVersionFilter=YesOrNo.FALSE, includeOperational=YesOrNo.FALSE, xlateOrg=None, xlateMap=None):
	...
	from MoMeta.OrgOrg import OrgOrg
	referenceDict = {}
	differenceDict = {}
